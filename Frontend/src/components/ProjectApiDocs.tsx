...
import { Terminal, Copy, Check, ChevronDown, ChevronRight, Database, ShieldAlert, Server, Info } from 'lucide-react';
...
const MethodBadge = ({ method }: { method: string }) => {
  ...
  const colorClass = colors[method as keyof typeof colors] || 'bg-gray-100 text-gray-700 border-gray-200';
  
  return (
    <span className={`px-2 py-1 text-xs font-bold rounded-md border ${colorClass}`}>
      {method}
    </span>
  );
};
    ...
    <div className="border border-gray-200 rounded-xl mb-4 overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <code className="font-mono text-sm text-gray-800 font-semibold">{endpoint.path}</code>
        </div>
        <div className="flex items-center gap-4">
          <span className="text-sm text-gray-500 hidden sm:inline-block">{endpoint.description}</span>
...
export function ProjectApiDocs({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-md mt-8">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">API Integration Guidelines</h2>
        <p className="text-gray-500">
          Use these endpoints to integrate authentication and data storage into your own frontend application.
        </p>
      </div>
      ...
      <div className="mb-8">
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 mt-1 mb-2">Prepend this base URL to all API requests below:</p>
        ...
        <div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
            ...
            <p className="text-sm text-amber-800 mt-2">
              The authentication API strictly enforces capitalized keys and formats. You <strong>must</strong> use these exact keys if providing these fields:
            </p>
            <ul className="list-disc list-inside text-sm text-amber-800 mt-2 space-y-1 ml-1">
              <li><code className="font-semibold font-mono bg-amber-100 px-1 rounded">Email</code>: Valid email address</li>
              <li><code className="font-semibold font-mono bg-amber-100 px-1 rounded">Name</code>: Only letters & 2-50 characters</li>
              <li><code className="font-semibold font-mono bg-amber-100 px-1 rounded">UserName</code>: Letters, numbers, _, ., 3-20 chars (no spaces)</li>
              <li><code className="font-semibold font-mono bg-amber-100 px-1 rounded">Password</code>: Min 8 chars, 1 uppercase, 1 lowercase, 1 number, 1 special char</li>
              <li><code className="font-semibold font-mono bg-amber-100 px-1 rounded">Phone</code>: Indian mobile numbers (10 digits)</li>
            </ul>
            
            <p className="text-sm font-semibold text-amber-900 mt-4 mb-2">Required Metadata Arrays:</p>
            <ul className="list-disc list-inside text-sm text-amber-800 space-y-1 ml-1">
              <li><code className="font-semibold font-mono bg-amber-100 px-1 rounded">Identifiers</code>: Array of strings specifying which keys are used for login (e.g., <code className="text-xs">["Email"]</code>). Required for Register & Login.</li>
              <li><code className="font-semibold font-mono bg-amber-100 px-1 rounded">uniqueField</code>: Array of strings specifying which keys must be unique in the database. Required for Register.</li>
            </ul>
          </div>
        ...
        <div>
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-6">
            ...
            <p className="text-sm text-emerald-800 mt-2">
              When adding user data via POST, you <strong>must</strong> wrap your arbitrary JSON data inside a root key named <code className="font-semibold font-mono bg-emerald-100 px-1 rounded">UserData</code>. The backend specifically looks for this exact key.
            </p>
          </div>
